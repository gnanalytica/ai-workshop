---
reading_time: 10 min
tldr: "Thirty days ago you didn't know where to start. Today you ship. Go time."
tags: ["launch", "demo-day", "graduation", "capstone"]
video: https://www.youtube.com/embed/T9aRN5JkmL8
lab: {"title": "THE DEMO — live panel", "url": "/showcase.html"}
prompt_of_the_day: "Write my 200-word 'what I'll do next' statement. My capstone is {{capstone}}, the hardest thing I learned was {{lesson}}, and my next 30-day goal is {{goal}}. Voice: confident, specific, no hype."
tools_hands_on: [{"name": "Your live capstone URL", "url": "/showcase.html"}, {"name": "Showcase page", "url": "/showcase.html"}, {"name": "Certificate", "url": "/certificate.html"}]
tools_demo: [{"name": "Instructor state-of-the-cohort", "url": "/showcase.html"}, {"name": "Awards ceremony", "url": "/certificate.html"}]
tools_reference: [{"name": "Certificate (auto-issued at 30/30)", "url": "/certificate.html"}, {"name": "Cohort showcase", "url": "/showcase.html"}]
resources: [{"name": "Your capstone repo", "url": "/showcase.html"}, {"name": "Your showcase entry", "url": "/showcase.html"}]
objective:
  topic: "Demo Day — ship the live capstone, graduate, enter the alumni network"
  tools: ["Your live capstone URL", "Showcase page", "Certificate"]
  end_goal: "Deliver the 5-minute panel demo, post the final submission (live URL + repo + deck + teaser + 200-word next statement), and download your certificate."
---

## 🎯 Today's objective

**Topic.** Demo Day. You pitch the capstone you shipped to a live panel, graduate, and step into the alumni network.

**Tools you'll use.** Your live capstone URL, the cohort [showcase](/showcase.html), the auto-issued [certificate](/certificate.html).

**End goal.** By the end of today you will have:
1. Delivered a 5-minute live panel demo (4 min + 1 min Q&A) with a pre-recorded backup ready.
2. Posted the final submission: live URL, repo, pitch deck, 30-second teaser, and 200-word "what I'll do next."
3. Downloaded your certificate, joined the alumni channel, and shared your showcase link on LinkedIn (#GnanalyticaAIWorkshop).

> *Why this matters:* Thirty days ago most of you had never written a prompt you were proud of. Today you ship — in front of a panel, with a URL anyone can open. That is not a small thing. That is the thing.

---

## ⏪ Pre-class · ~20 min

**Revision / context — the 30-day arc in six beats.**

- **Week 1 · Foundations (Days 1–5).** What AI is vs isn't, tokens + attention + weights, prompt-craft, hallucinations, your personal AI stack. You stopped being a passive user.
- **Week 2 · Exposure (Days 6–10).** Hands-on tours of every flavor of AI tool, ending with you picking the capstone problem you'd live with for twenty days.
- **Week 3 · Think & Tell (Days 11–15).** Design thinking, user interviews, wireframes, and a 1-page capstone spec. You earned the right to build.
- **Week 4 · Dev + AI Build (Days 16–20).** GitHub flow, RAG, local models, n8n automation, context engineering, CLAUDE.md. You learned to direct AI to build things for you.
- **Week 5 · Build, Ship, Agentic (Days 21–25).** Vibe-coded the v0 (Day 21), deployed it live (Day 22), built agents with tools + MCP (Day 23), multi-agent orchestration (Day 24), mini-demo ideathon (Day 25).
- **Week 6 · Launch (Days 26–30).** Ethics + safety (Day 26), model cards + benchmarks (Day 27), AI signal pipeline (Day 28), polish + final dress rehearsal (Day 29), demo day (Day 30).

Yesterday (Day 29) was your final dress rehearsal. Today is the performance.

### Quick glossary

- **Demo day** — the live, time-boxed showcase where each team pitches their capstone to a panel.
- **Panel** — instructor + 2 guest judges; they ask the sharp questions and hand out awards.
- **Certificate** — auto-issued at /certificate.html when your 30/30 checkpoint completes today.
- **Showcase** — the public cohort gallery at /showcase.html where your live URL lives on.
- **Alumni channel** — the post-cohort community you join the moment you graduate.

### Setup
- [ ] Arrive 10 minutes early. Seriously — not 2 minutes early.
- [ ] Bring your laptop, charger, water, and your best smile.
- [ ] Live URL tested on a fresh browser (incognito, different wifi if you can).
- [ ] Pre-recorded backup tab open and playable with sound.

### Primer (~5 min)
- **Read**: your own hook, out loud, once, in the mirror.
- **Watch** (optional): 2 minutes of a favourite demo — borrow the energy, not the words.

### Bring to class
- [ ] Deep breath. Bring your A-game.
- [ ] Your specific 30-second ask, memorised.
- [ ] Gratitude — message one cohort peer and say thanks before you walk in.

---

## 🎥 During class · 60 min live session

### Agenda

| Block | Time | What |
|---|---|---|
| Instructor opening + cohort celebration | 5 min | One sentence per team |
| Live demos to panel (rotating) | 40 min | ≈6 teams × 6 min each |
| Panel Q&A + awards | 10 min | Certificate auto-issued on completion |
| Closing + what's next | 5 min | Alumni channel, showcase links, goodbyes |

### In-class moments (minute-by-minute)

- **00:00 — Opening**: instructor's one-minute welcome, panel introductions, demo order drawn live. Cohort applauds every name as it's called.
- **00:05 — Team demos begin**: 5 minutes per team (4 min demo + 1 min panel Q&A), rotating through all teams. Stay in the room, watch every demo, cheer loudly.
- **00:45 — Panel Q&A round**: cross-team questions, judges probe the riskiest assumption of each capstone. Answers scored on honesty, not polish.
- **00:52 — Awards ceremony**: panel reads awards and citations aloud; certificate auto-issues on /certificate.html for everyone at 30/30.
- **00:58 — Close + group photo**: instructor's send-off, alumni channel link shared, showcase gallery goes public, group photo (non-negotiable).

### Read: Go Time (400 words)

The structure you have rehearsed works. Trust it.

**30-second hook.** Name the pain, name the human, make them lean in. The panel is seeing eight demos today. Yours needs to land in the first sentence or it won't land at all.

**2-minute story.** One end-to-end path. One real moment of value. No settings screens. No empty states. Pre-recorded backup ready on a second tab in case something breaks. If something does break, smile, say "of course, on stage" — laugh with them, switch tabs, keep going. Grace under failure is what the panel remembers.

**30-second ask.** Specific. Name it. Users, hires, capital, collaborators, feedback — whichever you actually want. Vague asks get forgotten. Sharp asks get followed up on Monday morning.

#### During Q&A

Listen to the whole question. Pause one beat. Answer the question that was actually asked, then bridge to your strongest point. If you don't know, say "I don't know yet — here's how I'd find out." That answer scores higher than a bluff every single time.

#### After your demo

Stay in the room. Watch every other demo. Applaud loudly. Cohorts that cheer each other on build careers together. Some of the people sitting next to you today will be your co-founders, your early hires, your best references, your lifelong group chat. That is not workshop sentimentality. That is the actual long-run value of showing up for 30 days.

#### The certificate

When your 30/30 checkpoint completes today, certificate.html auto-issues. Download it. Print it if you want. But know this: the certificate is a proof-of-finish, not a proof-of-skill. The proof-of-skill is the live URL in your demo. That URL is your portfolio now.

#### One last thing

The field does not stop moving because you graduated. Tomorrow morning, open your Day 28 pipeline. Friday, do your first 30-minute signal read. Next month, ship version 2. The workshop ends today. The work doesn't.

### Watch: State of the Cohort (10 min)

https://www.youtube.com/embed/T9aRN5JkmL8

Instructor opens Demo Day with a look back at what this cohort shipped — by the numbers, by the stories, by the names.

### Lab: THE DEMO (live panel)

- 5 minutes per team, rotating order drawn live.
- 4 minutes demo, 1 minute panel Q&A.
- Panel: instructor + 2 guest judges.
- Awards ceremony immediately after the last demo.
- Certificate auto-issues on certificate.html for everyone at 30/30.
- Group photo. Non-negotiable.

> ⚠️ **If you get stuck**
> - *Screen share won't connect when your turn comes* → don't debug on stage; switch immediately to your pre-recorded backup tab, narrate over it, and save the live URL for the Q&A. The panel will not penalize a graceful fallback.
> - *Your video freezes mid-demo* → pause, smile, say "of course, on stage" — laugh with the room, refresh once, and if it doesn't recover in 5 seconds, switch to the backup. Grace under failure is the story the panel remembers.
> - *A panel question lands you sideways* → listen fully, pause one beat, and if you genuinely don't know, say "I don't know yet — here's how I'd find out." That scores higher than a bluff every single time.

### Live discussion prompts — What you'll carry forward

| Prompt | What a strong answer sounds like |
|---|---|
| What you believed about AI on Day 1 that you no longer believe. | Names a specific belief in plain language — a tool you overrated, a fear you outgrew, a hype you bought — and the exact moment in the cohort that shifted it. Celebrate the change; you earned it. |
| What you believe now that you didn't then. | A concrete conviction you'll defend — a pattern, a principle, a non-negotiable — not a buzzword. One sentence is enough if it's sharp. |
| What you'll be working on in six months. | A shippable goal with a rough date and a real user in mind. "Version 2 of my capstone with 10 paying users by October" beats "keep learning." |

Read every single other post before you close the tab tonight — that is how you leave.

---

## 📝 Post-class · ~2 hour focused block

Block the evening. Phone on DND. Do these in order.

### 1. Immediate: FINAL SUBMISSION (~60 min)

Ship one submission containing:
- **Live capstone URL** (deployed, publicly accessible)
- **Repo link** (README with run instructions)
- **Pitch deck** (5-8 slides max)
- **30-second teaser video** (Loom or similar)
- **200-word "What I'll do next"** — concrete, dated, shippable goal for the next 30 days

Post in the cohort channel. Tag two people you want to keep building with.

Then:
1. Download your certificate from [/certificate.html](/certificate.html).
2. Share your certificate + [showcase link](/showcase.html) on LinkedIn with **#GnanalyticaAIWorkshop** — tag two peers.
3. Join the alumni channel (link in the closing slides). This is where the next 12 months happen.
4. Group photo uploaded and reshared.

### 2. Reflect (~5 min)
Write one sentence you'd tell Day-1 You. Pin it somewhere you'll see it in six months.

### 3. Quiz (~15 min)

1. What are the three beats of your demo? (Say them out loud before you go on.)
2. What do you do if the live API fails on stage?
3. What is your specific ask?
4. Where does your certificate auto-issue?
5. When is your first post-cohort signal-read session?

### 4. Submit (~5 min)
Confirm the cohort channel post landed with all five artifacts (URL, repo, deck, teaser, 200-word next statement). Confirm the showcase entry renders. Confirm the certificate downloaded.

### 5. Deepen (optional ~30 min)
- **Extra video**: rewatch your own demo in one week with fresh eyes — note what you'd change for v2.
- **Extra read**: [State of AI Report](https://www.stateof.ai) cover-to-cover on a quiet weekend.
- **Try**: write a short LinkedIn post about one specific thing you learned — not the workshop in general, *one thing*. That's the post that gets read.

### 6. What's next after Day 30 (~30 min)

**You just shipped a real thing.** Here is how you keep it alive.

#### What you built over 30 days — and what to do with it

- **A live capstone URL.** Keep it live. Put it in your LinkedIn header, your email signature, your GitHub pinned repos. The URL is your portfolio now — the certificate is proof-of-finish, the URL is proof-of-skill.
- **A public showcase entry.** The [cohort showcase](/showcase.html) goes public today. Your tile is a permanent link you can share for the next two years.
- **A GitHub repo with a real README.** Add a one-line project description, a demo GIF, the run instructions, and a link to the live URL. Recruiters, collaborators, and future-you will all land here first.
- **A certificate.** Download from [/certificate.html](/certificate.html) at 30/30. Print one for the wall; attach the PDF to applications; add the credential to your LinkedIn.
- **A 200-word "what's next" statement.** This is the spec for your next 30 days. Treat it like a contract with yourself.

#### Concrete next-step resources

- **This Friday**: your first 30-minute [Day 28 signal read](/day-28.html) — prove the habit is real.
- **Next week**: ship **version 2** of your capstone based on one panel question you couldn't answer cleanly.
- **Next month**: get 10 real users on that live URL. A cohort capstone that has never had a non-cohort user is incomplete.
- **Ongoing**: stay in the alumni channel, hire each other, reference each other, celebrate each other's launches. This cohort is a 10-year asset, not a 30-day cohort.

#### Advanced topics to go deeper on

- **Evals & benchmarks** — revisit [Day 27](/day-27.html) and build a real eval harness for your capstone. This is the single highest-leverage skill for the next two years.
- **Agents & multi-agent** — extend Day 23–24 work into a production agent for your v2. Re-read [Anthropic: Building effective agents](https://www.anthropic.com/engineering/building-effective-agents).
- **Safety & model cards** — before you onboard real users, revisit the [Day 26 safety audit](/day-26.html) and [Day 27 model card](/day-27.html).
- **Open vs closed weights** — pick one open-weight model from Day 17 and fine-tune it on a real dataset of yours.

#### Communities to join

- **The Gnanalytica alumni channel** — your first stop. Ship updates, ask for critique, hire.
- **AI Engineer / Latent Space** communities — podcasts, Discords, newsletters that keep you current.
- **Local AI meetups** in your city — the people who will be your co-founders and first hires.
- **Open-source projects** you used this month (LangGraph, MCP servers, CrewAI) — file one issue, send one PR. That is how you become known.

#### Projects to ship next

- **v2 of your capstone** — a focused refactor driven by the panel's sharpest question.
- **An MCP server** that wraps one internal tool or data source your team uses. (See [Day 23](/day-23.html).)
- **A Claude Skill or Plugin** that packages something you do repeatedly. (See [Day 24](/day-24.html).)
- **A short written artifact** — one blog post, one LinkedIn thread, one Loom teardown — on one specific thing you learned. Specific beats general every time.

Congratulations. You shipped. See you on the internet. #GnanalyticaAIWorkshop

---

## 📚 Extra / additional references

### Pre-class primers
- [Your showcase entry](/showcase.html)
- [Your live capstone URL](/showcase.html)

### Covered during class
- [Certificate](/certificate.html) — auto-issues at 30/30.
- [Cohort showcase gallery](/showcase.html) — goes public today.
- Panel Q&A, awards ceremony, group photo.

### Deep dives (post-class)
- [Day 28 — AI Signal Pipeline](/day-28.html) — your first Friday slot.
- [Day 26 — safety audit](/day-26.html) — revisit before you onboard real users.
- [Day 27 — model card](/day-27.html) — update when you swap models for v2.

### Other videos worth watching
- Your own demo, one week from now. Everything else is a distant second.
