...
/** Explains pod-scoped visibility for faculty/trainers (not global tech queue). */
export async function HelpDeskScopeNotice() {
  const isGlobalSupport = await checkCapability("support.tech_only");
  if (isGlobalSupport) return null;
  return (
    <div className="border-accent/25 bg-accent/5 text-ink/90 rounded-lg border px-4 py-3 text-sm leading-relaxed">
      ...
      <p className="text-muted mt-1">
        ...
        ask a lead to add you under Pod settings → Faculty.
      </p>
    </div>
  );
}
