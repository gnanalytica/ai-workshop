...
- [ ] **Antigravity** installed and signed in with your cohort Google account. Complete any other installs your cohort sent by email or chat. **Faculty:** use **Handbook → Setup & tools** (*Lab environment & tooling*) for a full lab checklist.
...
5-question quiz covering vibe-coding fundamentals — the core idea, picking the right tool for a quick browser prototype, fixing a common Windows install error, best practices, and matching a tool to a one-click full-stack build. Open it on your dashboard during class.

## 📝 Assignment · Vibe-coded portfolio

**Brief.** Use a vibe-coding tool (Google AI Studio, Bolt, Emergent, Lovable, or Antigravity) to build a **single-page personal portfolio**. It should include: a one-line bio, 2–3 projects, a contact section, and at least one styling choice you actively edited (colour, font, layout). Use Plan mode first — write a 4-step plan, edit it, then let the tool generate.

**Submit one of the following on the dashboard:**

1. **An image of your portfolio** — take a clean full-page screenshot, upload it to Google Drive / Imgur / GitHub (set to *anyone with the link*), and paste the **public image URL** in the Links field (label it `Portfolio screenshot`).
2. **A Google AI Studio share link with preview** — in AI Studio click **Share** → *anyone with the link*, copy the URL, and paste it in the Links field (label it `AI Studio preview`).

Either one counts as a valid submission. You can submit both if you want.

**Also include in the text box (60–120 words):**
- Which tool you picked and why.
- One thing vibe coding nailed on the first try.
- One thing it got wrong, and how you corrected it (re-prompt? hand-edit? switched tools?).

**Rubric.** Portfolio works / preview opens (4) · Plan was written and edited before code (3) · Honest reflection with a concrete failure example (3).

## 🔁 Prep for next class

Day 17 is next — check the dashboard for the topic and setup instructions.

- [ ] Polish your Day-13 deck.
