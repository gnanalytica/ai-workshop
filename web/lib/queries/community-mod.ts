...
export const listModCommunityPosts = cache(async (cohortId: string): Promise<ModPost[]> => {
  const sb = await getSupabaseServer();
  const { data } = await sb
    .from("community_posts")
    .select("id, cohort_id, title, pinned_at, deleted_at, created_at, profiles:author_id(full_name), community_replies(count)")
