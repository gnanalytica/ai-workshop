-- Transfer + spaced-retrieval quiz items. Run after quizzes_v2.sql (same cohort_id).
-- Appends two questions per day via jsonb array concatenation.
-- Idempotent: skips a day if that day's scenario stem question is already in `questions`.
